...
- プレビュー中は0.3秒おきにQRコード・バーコード(EAN-13/EAN-8/UPC-A/UPC-E等)を常時検出（選択範囲があればその中だけ、無ければ画面全体が対象）。前回とは異なる内容を検出したときのみテキストエリアに表示し、OCR結果の表示中はステータス行のOCR情報（ページ番号・文字数・処理時間）を`OCR: --`にリセットする。「クリップボードコピー」チェックボックスがONなら自動でクリップボードにコピーし、デスクトップ通知も送信する
- OCR完了時・画像保存時・QRコード/バーコード検出時にデスクトップ通知（`notify-send`）を送信
...
- `src/ocr_app/qr.py` — フレームからのQRコード検出（OpenCVの`QRCodeDetector`を使用）
- `src/ocr_app/barcode.py` — フレームからのバーコード検出（OpenCVの`barcode.BarcodeDetector`を使用）
